"""RL agent: tactical levers, reward shaping, decision audit."""
...
class BrentsRLAgent(SinglesEnv):
        ...
        for fainted_species in newly_fainted:
            ...
            for my_mon in my_team:
                for opp_move in fainted_mon.moves.values():
                    try:
                        _, max_pct = self.vector_builder.mechanics.damage_range_percent(
                            battle, fainted_mon, my_mon, opp_move, None, None,
                        )
                        if max_pct >= safe_hp_fraction(my_mon) and max_pct > 0.5:
                            ohko_count += 1
                            break
                    except Exception:
                        continue
    ...
    def _consume_tactical_shaping(self, battle: AbstractBattle) -> float:
        ...
        if context is None or context.battle_tag != battle_tag(battle):
            return 0.0
        self._tactical_reward_context = None
    ...
    def _evaluate_move_tactical_levers(
        ...
        for reason, reward_key, details, record_audit in (
            ("redundant_stealthrock", "penalty_redundant_stealthrock", self._evaluate_redundant_stealthrock(battle, move), False),
            ("redundant_stickyweb", "penalty_redundant_stickyweb", self._evaluate_redundant_stickyweb(battle, move), False),
            ("redundant_spikes", "penalty_redundant_spikes", self._evaluate_redundant_spikes(battle, move), False),
            ("redundant_status", "penalty_redundant_status", self._evaluate_redundant_status(battle, move), False),
            ("bad_encore", "penalty_bad_encore", self._evaluate_bad_encore(battle, move), False),
            ("ineffective_heal", "penalty_ineffective_heal", self._evaluate_ineffective_heal(battle, move), False),
            ("wasteful_heal_overflow", "penalty_wasteful_heal_overflow", self._evaluate_wasteful_heal_overflow(battle, move), True),
            ("redundant_self_drop_move", "penalty_redundant_self_drop_move", self._evaluate_redundant_self_drop_move(battle, move), True),
            ("unsafe_stay_in_with_fast_ko_switch", "penalty_unsafe_stay_in_with_fast_ko_switch", self._evaluate_unsafe_stay_in_with_fast_ko_switch(battle, move), True),
            ("good_heal_timing", "bonus_good_heal_timing", self._evaluate_good_heal_timing(battle, move), False),
            ("heal_satiation", "penalty_heal_satiation", self._evaluate_heal_satiation(battle, move), True),
            ("good_attack_selection", "bonus_good_attack_selection", self._evaluate_good_attack_selection(battle, move), False),
        ):
    ...
    def _evaluate_switch_tactical_levers(
        ...
        matches: list[TacticalLeverMatch] = []
        for reason, reward_key, details, record_audit in (
            ("good_safe_switch", "bonus_good_safe_switch", self._evaluate_good_safe_switch(battle, switch_target), False),
            ("abandon_boosted_mon", "penalty_abandon_boosted_mon", self._evaluate_abandon_boosted_mon(battle), True),
            ("wasted_free_switch", "penalty_wasted_free_switch", self._evaluate_wasted_free_switch(battle), True),
        ):
            match = self._make_tactical_match(reason, reward_key, details, record_audit=record_audit)
            if match is not None:
                matches.append(match)
    ...
    def _evaluate_good_tera(
        ...
        m = self.vector_builder.mechanics
        ...
        if opp_last_move is not None:
            ...
            if opp_move_type is not None:
                pre_tera_mult = opp_move_type.damage_multiplier(
                    ...
                    type_chart=m.gen_data.type_chart,
                )
        ...
        enables_ko = False
        move_type = m.resolve_move_type(active, move, battle)
        if move_type == tera_type and move.base_power and move.base_power > 0:
            ...
            if opp_hp > 0.0:
                ...
                post_stab = stab_multiplier(active, move_type)
                if post_stab > pre_stab:
                    try:
                        _, max_pct = m.damage_range_percent(
    ...
    def _evaluate_abandon_boosted_mon(
        ...
        speed_adv = self.vector_builder.mechanics.speed_advantage(battle)
        if speed_adv < 0.5:
            return None
    ...
    def _evaluate_heal_satiation(
        ...
        return {
            ...
            "hp_fraction": round(safe_hp_fraction(active), 3),
        }
    ...
    def _heal_summary(
        ...
        m = self.vector_builder.mechanics
        raw_heal = m.move_heal_amount(move, battle)
        if raw_heal <= 0.0:
            return None

        effective_heal = m.effective_move_heal_amount(move, active, battle)
        hp_fraction = safe_hp_fraction(active)
        return {
    ...
    def _evaluate_redundant_self_drop_move(
        ...
        opponent_hp = safe_hp_fraction(opponent)
        if opponent_hp > 0.0 and chosen_max >= opponent_hp:
            return None

        best_alt = self._best_move_choice(
            battle, active, opponent,
            exclude_move_id=move.id, damaging_only=True,
        )
    ...
    def _evaluate_good_attack_selection(
        ...
        best_move = self._best_move_choice(battle, active, opponent, damaging_only=True)
    ...
    def _move_expected_metrics(
        ...
        accuracy = clamp01(float(getattr(move, "accuracy", 1.0) or 1.0))
    ...
    def _move_has_unjustified_tradeoff(
        ...
        opponent_hp = safe_hp_fraction(defender)
        ...
        if self_boosts.get(relevant_stat, 0) < 0:
            return True
        if m.move_causes_recharge(move) >= 1.0:
            return True

        recoil = m.estimated_recoil_fraction(attacker, defender, move, expected_value)
    ...
    def _resists_all_threat_moves(
        ...
        for entry in threat_entries:
            move_type = m.resolve_move_type(attacker, entry.move, battle)
            type_mult = defender_type_mult(move_type, defender, m.gen_data.type_chart)
            if type_mult > 0.5:
                return False
        return True
    ...
    def _assess_battle_threats(self, battle: AbstractBattle) -> ThreatAssessment:
        ...
        threat_entries = tuple(
            entry
            for entry in m.select_opponent_threat_entries(opponent, posterior)
            if entry.move.category != MoveCategory.STATUS and float(getattr(entry.move, "base_power", 0) or 0) > 0.0
        )
        active_max_threat = 0.0
        if threat_entries:
            active_max_threat = max(
                m.move_expected_value(
                    battle, opponent, active, entry.move,
                    battle.opponent_role, battle.player_role,
                )
                for entry in threat_entries
            )

        active_ohko_risk = m.estimate_ohko_risk(battle, opponent, active, posterior)
        active_speed = m.effective_speed(active, battle.side_conditions)
        opponent_speed = m.effective_speed(
            opponent, battle.opponent_side_conditions, role_posterior=posterior,
        )
        return ThreatAssessment(
            posterior=posterior,
    ...
    def _switch_candidate_metrics(
        ...
        m = self.vector_builder.mechanics
        candidate_speed = m.effective_speed(candidate, battle.side_conditions)
        faster = (
        ...
        if threat.threat_entries:
            candidate_max_threat = max(
                m.move_expected_value(
                    battle, opponent, candidate, entry.move,
                    battle.opponent_role, battle.player_role,
                )
                for entry in threat.threat_entries
            )
            candidate_ohko_risk = m.estimate_ohko_risk(
                battle, opponent, candidate, threat.posterior,
            )

        best_reply = self._best_known_bench_reply(battle, candidate, opponent)
        return {
            "switch_species": getattr(candidate, "species", "<unknown>"),
            "switch_hp_fraction": round(safe_hp_fraction(candidate), 3),
            ...
            "resists_all_threats": self._resists_all_threat_moves(
                battle, opponent, candidate, threat.threat_entries,
            ) if threat.threat_entries else False,
        ...
        samples.append(
            {
                ...
                "active_hp_fraction": round(safe_hp_fraction(active), 3),
                "opponent_hp_fraction": round(safe_hp_fraction(opponent), 3),
                ...
                "bench_summary": [
                    {
                        ...
                        "hp_fraction": round(safe_hp_fraction(mon), 3),
